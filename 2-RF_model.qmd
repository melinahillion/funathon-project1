...
Update the preprocessing pipeline with this new model. 

Train the model (it should take about 10 minutes to train). 
...
::: {.callout-tip}
Scikit-learn allows you to access every step of the pipeline with `model`'s attributes. For example, to access the transformer you would type `model.transformer` and to access the regressor you would type `model.regressor`. 


```python
model.regressor   # allows to access the unfitted pipeline defined with rf_pipeline
```

To access one of the two steps defined in the rf_pipeline (preprocessing or random_forests), they are stored in the dictionnary `model.regressor`. 

```python
model.regressor["preprocessing"]   # access to the first step
model.regressor["random_forests"]   # access to the second step
```

:::

::: {.callout-important}
Scikit-learn  distinguishes **parameters** from **fitted attributes**. Here, when we define the `model` step : 

- `regressor` is the parameter we pass **before** fitting. It's the estimator we configure.
- `regressor_` is the attribute set **after** calling `.fit()`. It's a clone of `regressor` that has been actually trained on data.

The `_` suffix is a consistent scikit-learn convention: any attribute that only exists after fitting ends with `_` (e.g., `coef_`, `n_features_in_`, `classes_`). It signals "this was learned from data."

```python
model.regressor_  # allows you to access the fitted clone of rf_pipeline (after model.fit())
```

To access the attributes sent back by the training phase listed in the documentation, you therefore need to call 

```python
# This would fail — the original rf_pipeline is unfitted
model.regressor['random_forests'].oob_score_  # AttributeError

# Access the fitted RF (has .feature_importances_, .oob_score_, etc.)
fitted_rf = model.regressor_['random_forests']
...
model.regressor_['random_forests'].oob_score_
```
...
::: {.callout-tip collapse="false" icon="false"}
## <i class="bi bi-book"></i> Exercice 5: Tuning RF hyperparameters

Now that you understand what a RF is, try to tune its hyperparameters for training. In this exercise, your aim is to train the best model for the prediction of price per square meter.
...
1.  Subsample a fraction ( `0.1` ) of the dataset to reduce computation time.
...
subsample = 0.1 # 10% of the dataset for the plot
